...
public class PlayerHostedGameTest {

    public PlayerHostedGameTest() {
    }
    ...
    public void shouldPerformFewMovesOnAMap() throws ShipGameException, IOException, InterruptedException {
        ...
        PlayerHostedGame sut = new PlayerHostedGame(playerMap, opponentMap, playerMapView, opponentMapView, serverConnection);
        verify(opponentMapView).addFieldSelectObserver(captor.capture());
        verify(serverConnection).setPlayerMoveQueue(playerQueueCaptor.capture());
        verify(serverConnection).setOpponentMoveQueue(opponentQueueCaptor.capture());
